...
# endif
        ...
        [Tooltip("Verplicht: sleep hier een GameObject in de scene naartoe dat als spawnpunt " +
                 "van de EERSTE speler (host) dient. De positie EN rotatie (Z-as = kijkrichting) " +
                 "van dit object bepalen waar/hoe de rij spelers start. Zet dit object fysiek " +
                 "op de plek waar de eerste speler moet staan voordat de productie start.")]
        [SerializeField] Transform m_FirstSpawnPoint;

        [Tooltip("Richting waarin volgende spelers achter elkaar spawnen (bv. (0,0,-1) = " +
                 "naar achteren), relatief t.o.v. de rotatie van m_FirstSpawnPoint. Moet " +
                 "overeenkomen met de richting waarin studenten zich fysiek opstellen.")]
        [SerializeField] Vector3 m_SpawnLineDirection = new Vector3(0f, 0f, -1f);

        [Tooltip("Afstand in meters tussen elke speler in de rij.")]
        [SerializeField] float m_SpawnSpacing = 0.8f;

        /// <summary>
        /// Rij-index (0 = host/eerste speler, 1 = tweede speler, etc.) die de VOLGENDE speler
        /// die joint zou krijgen. Wordt ook gebruikt om de lokale XR-rig van die speler
        /// (niet enkel diens netwerk-avatar) op de juiste spawnpositie te zetten.
        /// </summary>
        public int NextSpawnIndex => m_CurrentPlayerIDs.Count;

        /// <summary>
        /// Berekent de spawn-positie in de rij op basis van de volgorde van binnenkomst
        /// (0 = host/eerste speler, 1 = tweede speler, etc.), relatief aan m_FirstSpawnPoint.
        /// </summary>
        public Vector3 GetSpawnPositionForIndex(int index)
        {
            if (m_FirstSpawnPoint == null)
            {
                Utils.Log($"{k_DebugPrepend}m_FirstSpawnPoint is niet ingesteld op de XRINetworkGameManager. " +
                          "Spelers spawnen op (0,0,0).", 2);
                return Vector3.zero;
            }

            Vector3 direction = m_FirstSpawnPoint.TransformDirection(m_SpawnLineDirection.normalized);
            return m_FirstSpawnPoint.position + direction * (m_SpawnSpacing * index);
        }

        public Quaternion GetSpawnRotation()
        {
            return m_FirstSpawnPoint != null ? m_FirstSpawnPoint.rotation : Quaternion.identity;
        }
