...
namespace XRMultiplayer
{
    ...
    public class XRINetworkPlayer : NetworkBehaviour
    {
        ...
        public override void OnNetworkSpawn()
        {
            ...
            if (IsLocalPlayer)
            {
                ...
                if (m_XROrigin != null)
                {
                    ...
                    // Het daadwerkelijk in de scene gebruikte rig-prefab ("XR Origin Hands (XR
                    // Rig) MP Template Variant") erft `m_RequestedTrackingOriginMode:
                    // NotSpecified` van zijn basisprefab (XRI Starter Assets), NIET Floor. Zonder
                    // expliciet Floor aan te vragen valt dit terug op wat de runtime toevallig
                    // als huidige mode rapporteert; als dat "Device" is, telt XROrigin
                    // m_CameraYOffset (~1.36m) op bij de camerahoogte, waardoor de vloer virtueel
                    // op ~navelhoogte komt te staan i.p.v. op de echte vloer.
                    m_XROrigin.RequestedTrackingOriginMode = XROrigin.TrackingOriginMode.Floor;
                    Debug.Log($"[SpawnDebug] TrackingOriginMode: Requested={m_XROrigin.RequestedTrackingOriginMode} " +
                              $"Current={m_XROrigin.CurrentTrackingOriginMode} CameraYOffset={m_XROrigin.CameraYOffset}");

                    // De XR-rig zelf (de fysieke ruimte/camera van de speler) staat los van dit
                    // netwerk-avatar-object en wordt dus NIET automatisch meeverplaatst wanneer
                    // XRINetworkGameManager.PlayerJoined() enkel player.transform (het avatar)
                    // positioneert. Zonder dit stond elke speler fysiek op (0,0,0) i.p.v. op de
                    // ingestelde m_FirstSpawnPoint-rij.
                    int spawnIndex = XRINetworkGameManager.Instance.NextSpawnIndex;
                    Vector3 spawnPos = XRINetworkGameManager.Instance.GetSpawnPositionForIndex(spawnIndex);
                    Quaternion spawnRot = XRINetworkGameManager.Instance.GetSpawnRotation();
                    Debug.Log($"[SpawnDebug] spawnIndex={spawnIndex} berekende spawnPos={spawnPos} spawnRot(euler)={spawnRot.eulerAngles}");
                    m_XROrigin.transform.SetPositionAndRotation(spawnPos, spawnRot);
                    Debug.Log($"[SpawnDebug] direct na SetPositionAndRotation: rig.position={m_XROrigin.transform.position} rig.rotation(euler)={m_XROrigin.transform.rotation.eulerAngles}");
                    float camHeightAboveRig = m_HeadOrigin.position.y - m_XROrigin.transform.position.y;
                    Debug.Log($"[SpawnDebug] camera.position.y={m_HeadOrigin.position.y} rig.position.y={m_XROrigin.transform.position.y} " +
                              $"cameraHeightAboveRig={camHeightAboveRig} (dit is de effectieve 'vloerhoogte'-fout als >0 en te hoog)");
                    var floorObj = GameObject.Find("Floor (1)");
                    Debug.Log(floorObj != null
                        ? $"[SpawnDebug] Floor (1) wereldpositie: {floorObj.transform.position}"
                        : "[SpawnDebug] Floor (1) niet gevonden via GameObject.Find (mogelijk andere naam/inactief)");
                    StartCoroutine(LogSpawnPositionDelayed(spawnPos));
                }
                else
                {
        ...
        /// <summary>
        /// Tijdelijke debug-hulp om te achterhalen waarom de rig-positie na het spawnen soms
        /// nog afwijkt van de berekende spawn-positie. Logt de rig-positie op +0,5s en +2s om
        /// te zien of iets (bv. een XR-recenter-event) de positie na de spawnfix alsnog verschuift.
        /// </summary>
        IEnumerator LogSpawnPositionDelayed(Vector3 expectedPos)
        {
            yield return new WaitForSeconds(0.5f);
            Debug.Log($"[SpawnDebug] +0.5s: rig.position={m_XROrigin.transform.position} " +
                      $"(verwacht {expectedPos}, verschil={Vector3.Distance(m_XROrigin.transform.position, expectedPos):F2}m) " +
                      $"CurrentTrackingOriginMode={m_XROrigin.CurrentTrackingOriginMode} " +
                      $"cameraHeightAboveRig={m_HeadOrigin.position.y - m_XROrigin.transform.position.y}");

            yield return new WaitForSeconds(1.5f);
            Debug.Log($"[SpawnDebug] +2.0s: rig.position={m_XROrigin.transform.position} " +
                      $"(verwacht {expectedPos}, verschil={Vector3.Distance(m_XROrigin.transform.position, expectedPos):F2}m) " +
                      $"CurrentTrackingOriginMode={m_XROrigin.CurrentTrackingOriginMode} " +
                      $"cameraHeightAboveRig={m_HeadOrigin.position.y - m_XROrigin.transform.position.y}");
        }
